...
useHead({
  title: computed(() => `${props.content.title} | Marlblue`),
  meta: [
    { name: "description", content: computed(() => props.content.description || "") },
  ],
});
...
const nextProject = computed(() => {
  ...
  const currentIndex = previews.findIndex((p) => p.slug === props.projectId);
...
<template>
  <Layout class="project-content">
    <ProjectHero :content="props.content" :projectId="props.projectId" />
    <div class="project-content-components">
      <div
        v-for="(component, index) in props.content.components"
